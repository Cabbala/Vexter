...
- Dexter remains the default execution anchor because it owns the broader operational objective set: opportunity capture, low-slippage entry, realized exit capture, and stale-risk avoidance.
- Mew-X remains a bounded selective option only for containment-first sleeves that explicitly prioritize drawdown containment, fast post-attempt fill, and local-peak retention.
- Hybrid use is supported only as objective-conditioned route selection or portfolio split, not as in-run source fusion.
