...
- Reused the fixed comparison, strategy, and execution-planning outputs already visible in Vexter.
- Reaggregated only the minimum design surface needed to make implementation boundaries explicit.
- Tested whether current evidence is enough to cut the first implementation-facing spec.

## Implementation Planning Read

- Planner/router decisions are now bounded to ex ante sleeve routing with Dexter as the default anchor.
- Mew-X remains a bounded containment-first selective sleeve with explicit enablement and timeout-aware monitoring.
- Portfolio design is bounded to single-anchor default, selective overlay, or dedicated containment template rather than blended in-run fusion.
