...
- Current result remains `FAIL/BLOCKED` because the canonical manifest is `template_only` and the repo still does not point to one current, fresh-enough, reviewable locator per required face, so the regenerated current pack cannot reopen retry-gate review honestly.
