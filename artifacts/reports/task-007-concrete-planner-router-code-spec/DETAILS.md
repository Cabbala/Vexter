...
- Reuses the fixed concrete planner/router implementation flow instead of reopening comparison work
- Converts that flow into code-ready Python module, data-model, and function boundaries
- Fixes rollback semantics, failure surfaces, and executor adapter seams without changing frozen source logic
...
## Core Code-Spec Result

- The implementation root is fixed as `vexter/planner_router/`
...
- Immutable plan and batch objects are frozen records, while source-native reasons remain passthrough metadata
- Dispatch uses `prepare` all before `start`, with stop-all rollback on partial prepare or start failure
- Dexter and Mew-X adapters remain source-faithful wrappers around existing config, session, buy, sell, and instrumentation surfaces
...
- Continue with `planner_router_code_implementation` as the next task.
- Carry forward `planner_router_code_spec_ready` as the current implementation conclusion.
- Preserve `code_spec_bounded` until monitor thresholds and executor transport are specified separately.
