...
- Reuses the fixed implementation-planning conclusion rather than reopening comparison work
- Converts the bounded implementation surface into the first immutable planner/router contract
- Fixes objective profile, sleeve, budget, monitor, and executor bindings inside one plan object
...
- `planner_router_spec_bounded`

## Core Interface Result

- Route by one sleeve before execution starts
- Keep Dexter as the default execution anchor
- Keep Mew-X as an explicit containment-first selective sleeve only
- Bind route, sleeve, budget, monitor profile, executor profile, and source pin into one immutable plan object
- Keep monitor profiles bound by reference, not by numeric threshold in this task
...
- Continue with `concrete_planner_router_implementation_spec` as the next task.
- Carry forward `planner_router_spec_ready` as the current implementation-level conclusion.
- Preserve `planner_router_spec_bounded` until a later spec fixes numeric budget caps, numeric monitor thresholds, and transport details.
