...
- Reuses the fixed planner/router implementation and the merged integration smoke instead of reopening comparison or rewriting source logic
- Adds bounded runtime smoke for `plan_request()` and `plan_and_dispatch()`
- Proves runtime status progression, typed quarantine/stop signaling, store ordering, and rollback completion with runtime-like stub adapters
- Leaves monitor numerics and executor transport/process ownership deferred while clearing the runtime-side seam question
...
- `runtime_smoke_bounded`

## Core Result

- `plan_request()` now has direct runtime-side smoke evidence for `planned -> starting -> running -> quarantined -> stopping -> stopped`
- `plan_and_dispatch()` now has direct runtime-side smoke evidence for invalid runtime transition handling and reverse-order stop-all rollback
- quarantine and stop remain typed seam outputs rather than planner-owned transport behavior
...
- Continue with `monitor_killswitch_spec` as the next task.
- Carry forward `planner_router_runtime_smoke_passed` as the current evidence conclusion.
- Preserve `runtime_smoke_bounded` while monitor escalation and live-paper behavior remain separate questions.
