...
OUTPUT_PATH="${1:-/Users/cabbala/Downloads/vexter_attestation_record_pack_regeneration_bundle_latest.tar.gz}"
BUNDLE_DIR_NAME="vexter_attestation_record_pack_regeneration_bundle_latest"
...
proof = json.loads(
    (root / "artifacts/proofs/demo-forward-supervised-run-retry-gate-attestation-record-pack-regeneration-check.json").read_text()
)
...
result_text = "\n".join(
    [
        "# ATTESTATION-RECORD-PACK-REGENERATION Result",
        "",
        f"- Result: {outcome}",
...
if all(named_summaries.values()):
    lines = ["# ATTESTATION-RECORD-PACK-REGENERATION Sub-agent Summaries", ""]
...
# Keep a tiny machine-readable breadcrumb for downstream inspection.
