...
10. Future analysis and validation should prefer paper or sim-safe collection modes over funded live trading when those modes are available. Mew-X should default to `sim_live`; Dexter should use explicit `paper_live` now that the formal paper mode is merged on Dexter `main`.
