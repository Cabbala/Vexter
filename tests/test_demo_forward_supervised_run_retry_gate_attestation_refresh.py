...
def test_demo_forward_supervised_run_retry_gate_attestation_refresh_artifacts_are_current_and_consistent() -> None:
    ...
    assert manifest["next_task"]["id"] == "DEMO-FORWARD-SUPERVISED-RUN-RETRY-GATE-ATTESTATION-RECORD-PACK"
    ...
    assert proof["verified_github"]["latest_vexter_pr"] == 78
    assert (
        proof["verified_github"]["latest_vexter_main_commit"]
        == "c8b586232c07ddf374d62f2817caeddb23581bd9"
    )
    assert proof["task_result"]["outcome"] == "FAIL/BLOCKED"
    assert proof["task_result"]["recommended_next_step"] == "supervised_run_retry_gate_attestation_record_pack"
    ...
    assert prompt_context["recommended_next_step"] == "supervised_run_retry_gate_attestation_record_pack"
    ...
    assert "baseline_attestation_record_pack_task_state" in handoff_text
