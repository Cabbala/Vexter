...
def test_demo_forward_supervised_run_retry_gate_attestation_refresh_artifacts_are_current_and_consistent() -> None:
    ...
    assert (
        ...
        == "/Users/cabbala/Downloads/vexter_next_task_bundle_post_pr103_retry_ff9c2e2f.tar.gz"
    )
    ...
    assert proof["verified_github"]["latest_vexter_pr"] == 104
    assert (
        proof["verified_github"]["latest_vexter_main_commit"]
        == "7a272b0e4367204397d6c4c87340365b35a599b8"
    )
    assert proof["verified_github"]["latest_vexter_merged_at"] == "2026-03-28T16:35:23Z"
    ...
    assert canonical_external_evidence["consistency_checks"]["template_only_false_path_holds"] is True
    assert (
        canonical_external_evidence["next_human_pass"]["bounded_window_fields_once"]
        == [
    ...
    assert canonical_external_evidence["next_human_pass"]["faces"][0]["face"] == "external_credential_source_face"

    refresh_boundary = context["evidence"]["demo_forward_supervised_run_retry_gate_attestation_refresh"][
        "attestation_refresh_boundary"
    ]
    assert context["evidence"]["github_latest"]["latest_recent_vexter_prs"] == [104, 103, 102, 101, 100]
    assert (
        context["evidence"]["github_latest"]["vexter_pr_104_merged_at"]
        == "2026-03-28T16:35:23Z"
    )
    ...
    assert refresh_evidence["external_evidence_preflight_report"] == PREFLIGHT_REPORT_REL_PATH
    assert refresh_evidence["external_evidence_preflight_proof"] == PREFLIGHT_PROOF_REL_PATH
    assert refresh_evidence["external_evidence_preflight_summary"] == PREFLIGHT_SUMMARY_REL_PATH
    assert refresh_evidence["external_evidence_preflight_status"] == "blocked"
    ...
    assert refresh_evidence["aggregated_blocked_reason_counts"]["template_only_manifest"] == 9
    assert "Manifest remains template_only" in refresh_evidence["template_only_false_path"]
    assert refresh_evidence["consistency_checks"]["blocked_reason_counts_match_face_rows"] is True
    assert refresh_evidence["next_human_pass"]["faces"][0]["face"] == "external_credential_source_face"
    ...
    assert "demo-forward-supervised-run-retry-gate-attestation-record-pack-regeneration-check.json" in " ".join(
        first_face["proof_paths_to_recheck"]
    )
    ...
    assert "template-only false path" in report_text
...
def test_export_attestation_refresh_closeout_bundle(tmp_path: Path) -> None:
    ...
    with tarfile.open(output_path, "r:gz") as tar:
        ...
        assert any(name.endswith("/NEXT_HUMAN_PASS.md") for name in names)
        ...
        next_human_pass_member = next(name for name in names if name.endswith("/NEXT_HUMAN_PASS.md"))
        ...
        next_human_pass_text = tar.extractfile(next_human_pass_member).read().decode("utf-8")
    ...
    assert "Reopen false-path" in next_human_pass_text
    assert "Consistency Checks" in next_human_pass_text
    for name in ("Anscombe", "Euler", "Parfit"):
