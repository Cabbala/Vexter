...
def test_demo_forward_supervised_run_retry_gate_input_attestation_artifacts_are_current_and_consistent() -> None:
    ...
    assert manifest["task_id"] == "DEMO-FORWARD-SUPERVISED-RUN-RETRY-GATE-ATTESTATION-RECORD-PACK-REGENERATION"
    assert (
        manifest["status"]
        == ledger["status"]
    ...
    assert (
        context["evidence"]["demo_forward_supervised_run_retry_gate_attestation_record_pack_regeneration"]["task_state"]
        == "supervised_run_retry_gate_attestation_record_pack_regeneration_blocked"
    )
