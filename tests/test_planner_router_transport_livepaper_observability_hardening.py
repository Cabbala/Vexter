...
class ObservabilityHardeningPlanStore:
...
class MirroringReconcilingStatusSink(ReconcilingStatusSink):
    def __init__(self, store: ObservabilityHardeningPlanStore) -> None:
        super().__init__()
        self.store = store
...
class ObservabilityHardeningTransportAdapter(SourceFaithfulTransportAdapter):
...
def build_observability_registry(
    ...
    *,
    ...
    dexter_adapter = ObservabilityHardeningTransportAdapter(
    ...
    mewx_adapter = ObservabilityHardeningTransportAdapter(
        observation_log=observation_log,
        source=Source.MEWX,
...
def record_snapshot(
    plan,
    store: ObservabilityHardeningPlanStore,
...
def test_transport_livepaper_observability_hardening_keeps_runtime_metadata_complete(
    tmp_path: Path,
) -> None:
    config_root = clone_config_dir(tmp_path)
    enable_mewx_sleeve(config_root)
    observation_log: list[dict[str, object]] = []
    store = ObservabilityHardeningPlanStore(observation_log)
    ...
    dispatch_snapshots = asyncio.run(
        plan_and_dispatch(
            PlanRequest(
                plan_request_id="req-transport-livepaper-observability-hardening-overlay",
...
def test_transport_livepaper_observability_hardening_keeps_failure_metadata_complete(
    tmp_path: Path,
) -> None:
    config_root = clone_config_dir(tmp_path)
    enable_mewx_sleeve(config_root)
    ...
    snapshots = asyncio.run(
        plan_and_dispatch(
            PlanRequest(
                plan_request_id="req-transport-livepaper-observability-hardening-timeout",
                objective_profile_id="dexter_with_mewx_overlay",
            ),
            config_root,
            FROZEN_PINS,
...
def test_transport_livepaper_observability_hardening_proof_tracks_regression_pack_as_next_step() -> None:
    proof = json.loads(
        (
            REPO_ROOT
            / "artifacts"
            / "proofs"
            / "task-007-transport-livepaper-observability-hardening-check.json"
        ).read_text()
    )

    assert proof["task_id"] == "TASK-007-TRANSPORT-LIVEPAPER-OBSERVABILITY-HARDENING"
    assert proof["verified_github"]["latest_vexter_pr"] == 49
    assert proof["transport_livepaper_observability_hardening"]["hardening_axes"][
        "status_sink_fan_in_completeness"
    ] is True
    assert proof["transport_livepaper_observability_hardening"]["runtime_surface"][
        "prepared_handles_include_handoff_metadata"
    ] is True
    assert proof["task_result"]["task_state"] == "transport_livepaper_observability_hardening_passed"
    assert proof["task_result"]["recommended_next_step"] == "transport_livepaper_observability_regression_pack"
